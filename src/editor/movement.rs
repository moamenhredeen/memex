...
impl EditorState {
    pub fn prev_grapheme(&self, offset: usize) -> usize {
        ...
        let char_idx = self.buffer.document.buffer.byte_to_char(offset);
        if char_idx == 0 {
            return 0;
        }
        self.buffer.document.buffer.char_to_byte(char_idx - 1)
    }
    ...
    pub fn next_grapheme(&self, offset: usize) -> usize {
        ...
        let char_idx = self.buffer.document.buffer.byte_to_char(offset);
        if char_idx + 1 >= self.buffer.document.buffer.len_chars() {
            return len;
        }
        self.buffer.document.buffer.char_to_byte(char_idx + 1)
    }
    ...
    pub(crate) fn offset_from_utf16(&self, utf16_offset: usize) -> usize {
        ...
        for ch in self.buffer.document.buffer.chars() {
